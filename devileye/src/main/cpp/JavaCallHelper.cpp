...
JavaCallHelper::JavaCallHelper(JavaVM *_javaVM, JNIEnv *_env, jobject &_jobj) : javaVM(_javaVM),
    ...
    jmid_on_result = env->GetMethodID(jSdkClass, "onDecodeCallback", "(Ljava/lang/String;I)V");
...
void JavaCallHelper::onResult(const std::string text, int scanType) {
    if (text.empty()) {
        return;
    }
...
//    }
    ...
    mJstring = env->NewStringUTF(text.c_str());

    env->CallVoidMethod(jSdkObject, jmid_on_result, mJstring, scanType);
